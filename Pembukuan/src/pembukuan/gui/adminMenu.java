    ...
    private void initComponents() {
        ...
        pembukuanLayout.setHorizontalGroup(
            ...
            .addGroup(pembukuanLayout.createSequentialGroup()
                .addContainerGap()
                .addGroup(pembukuanLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
        ...
        pembukuanLayout.setVerticalGroup(
            ...
            .addGroup(pembukuanLayout.createSequentialGroup()
                .addContainerGap()
                .addComponent(penjualanPane, javax.swing.GroupLayout.PREFERRED_SIZE, 300, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void btnHapusBarangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnHapusBarangActionPerformed
        ...
        if (idpilih != 0) {
            ...
            try {
                ...
                c.setConnection();
                ResultSet rs;
                rs = c.statement.executeQuery("select * from barang where id = '"+ idpilih +"'");
                if(rs.next()){
                    idPack = rs.getString("id_paket");
                }
                c.statement.executeUpdate("delete from barang where id = '" + idpilih + "'");
                
                rs = c.statement.executeQuery("select id,nama_barang,harga_barang,jumlah_barang from barang where id_paket ='"+idPack+"'");
                int row = tab.getRowCount();
                for (int i = 0; i < row; i++) {
                    tab.removeRow(0);
                }
...
//        });
    ...
    private void totActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_totActionPerformed
        ...
        try {
            ...
            c.setConnection();
            ResultSet rs;
            rs = c.statement.executeQuery("select sum(harga_barang) as HARGA from tabel_masuk ");
            while (rs.next()) {
                //                id_barang = rs.getString("id");
                //                nama_barang = rs.getString("nama_barang");
                totalBeli = rs.getInt("HARGA");
                //                jumlah_barang = rs.getString("jumlah_barang");

//                JOptionPane.showMessageDialog(null,
//                    "Rp   " + totalBeli,
//                    "Pengeluaran", JOptionPane.INFORMATION_MESSAGE);
                //                String[] data = {id_barang, nama_barang, jumlah_barang, harga_barang};
                //                tab.addRow(data);
            }
            
            rs = c.statement.executeQuery("select sum(harga_barang) as HARGA from tabel_keluar ");
            while (rs.next()) {
                //                id_barang = rs.getString("id");
                //                nama_barang = rs.getString("nama_barang");
                totalJual = rs.getInt("HARGA");
                //                jumlah_barang = rs.getString("jumlah_barang");
                
                total = totalJual - totalBeli;
